...
```python
from mempalace.db import open_collection, detect_backend, LanceCollection, ChromaCollection
```
...
#### open_collection(palace_path, collection_name="mempalace_drawers", backend=None, embedder=None, create=True)

Open or create a palace collection. Returns a `LanceCollection` or `ChromaCollection`.

```python
col = open_collection("~/.mempalace/palace")
col = open_collection("~/.mempalace/palace", backend="lance", embedder=my_embedder)
```
...
```python
from mempalace.embeddings import get_embedder, OnnxEmbedder, SentenceTransformerEmbedder, OllamaEmbedder
```
...
```python
from mempalace.sync import SyncEngine, ChangeSet, SyncRecord
```
...
col = open_collection("~/.mempalace/palace")
engine = SyncEngine(col, identity=NodeIdentity(), vv_path="~/.mempalace/palace/version_vector.json")
...
```python
from mempalace.sync_meta import NodeIdentity, get_identity, inject_sync_meta
