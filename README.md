...
MemPalace stores your AI conversations and project files in a local vector database (LanceDB), organized into a navigable structure of **wings** (projects/people), **rooms** (topics), and **drawers** (verbatim text). A companion **knowledge graph** (SQLite) tracks entity relationships with temporal validity.

Your AI assistant connects to MemPalace via [MCP](docs/mcp-server.md) and gets persistent memory across sessions — it can search past conversations, recall decisions, and track how facts change over time. Everything runs locally on your machine. Multi-device sync keeps your palace consistent across machines.
...
```bash
pip install mempalace[chroma]     # legacy ChromaDB backend (for migration)
pip install mempalace[gpu]         # GPU-accelerated embeddings via sentence-transformers
pip install mempalace[server]      # sync server (FastAPI + uvicorn)
```
...
Filtering by wing + room yields up to +34% retrieval improvement over unfiltered search (measured on 22,000+ real memories, using standard metadata filtering).
...
| [Sync](docs/sync.md) | Multi-device replication, server setup, conflict resolution |
